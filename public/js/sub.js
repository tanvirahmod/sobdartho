...
let firstNumber = function(digit,numberSystem,numberPosition) {
  if (digit < 0 || digit[0] === "-") {
    ...
    let zeros = repeat.repeat(8 - b.length) + b;
...
let secondNumber = function(digit,numberSystem,numberPosition) {
  ...
  if (digit[0] === "-") {
    ...
    an = digit;
    digit = "-" + an;
  }else if(digit === "a" || digit === "b" || digit === "c" || digit === "d" || digit === "e" || digit === "f" ){
    an = digit;
    digit = "-" + an;
  }else if(digit === "A" || digit === "B" || digit === "C" || digit === "D" || digit === "E" || digit === "F" ){
    an = digit;
...
// showing top details to interchange sings
  document.querySelector(".same").textContent = `(${document.querySelector(".firstDigit").value}) - (${document.querySelector(".secondDigit").value})`;
  let extra = document.querySelector(".secondDigit").value;
  let sign;
  if(extra < 0){
    sign = "+" + Math.abs(extra);
  }else if(extra >= 0){
    sign = "-" + Math.abs(extra);
  }else if(parseInt(extra, 16) < 0){
    sign = "+" + Math.abs(parseInt(extra, 16)).toString(16);
  }else if(parseInt(extra, 16) > 0){
    sign = "-" + Math.abs(parseInt(extra, 16)).toString(16);
  }
  document.querySelector(".change").textContent = `যোগ বানিয়েঃ (${document.querySelector(".firstDigit").value}) + (${sign})`
  ...
  if(secondAnswer != null){
    let hextoDecimal = document.querySelector('.secondDigit').value;
    let negNum2 = document.querySelector('.secondDigit').value > 0 || document.querySelector('.secondDigit').value[0] === "+" || parseInt(hextoDecimal, 16) > 0 ?"- " + document.querySelector('.secondDigit').value + " =" : "";
    document.querySelector(".oddEven2").textContent = `${negNum2.replace("+","")} ${secondAnswer.textContent}`;
  }
...
console.log(f,s);
  ...
  let zeroo = "0";
  // filling up all empty with zeros
  let withzero = zeroo.repeat(8 - binarySum.length) + binarySum;
  if(withzero[0] === "0"){
      ...
      // filling up all empty with zeros
      let withzero2 = zeroo.repeat(8 - finala.toString(2).length) + finala.toString(2);
